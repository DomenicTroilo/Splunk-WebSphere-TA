  ...
  do
    ...
    do
      ...
      if [[ $found -eq 1 ]]; then
        ...
        $javarun -Dhttps.protocols="TLSv1,TLSv1.1,TLSv1.2" -Dcom.ibm.CORBA.ConfigURL=file:${splunkdir}/sas.client.props.${port} -Dcom.ibm.SSL.ConfigURL=file:${propshome}ssl.client.props -cp ${splunkdir}/lib/jython-standalone-2.7.0.jar:${splunkdir}/lib/restConnector.jar:${splunkdir}/lib/com.ibm.ws.orb_8.5.0.jar:${splunkdir}/lib/com.ibm.ws.admin.client_8.5.0.jar:${splunkdir}/lib/com.ibm.ws.ejb.thinclient_8.5.0.jar:${splunkdir}/lib/javax.management.j2ee-api-1.1.1.jar org.python.util.jython ${splunkdir}/$collectlist $username $password $port 2>/dev/null
      fi
    done
  done
else
